...
function DeanDashboard() {
  ...
  const deanName = session?.user?.name || 'Dean'

  async function loadDashboard() {
    try {
      const [statsRes, eventsRes] = await Promise.all([
        fetch('/api/dean/stats', { cache: 'no-store' }),
        fetch('/api/dean/events?filter=pending&limit=6', { cache: 'no-store' }),
      ])
  ...
  useEffect(() => {
    ...
    const handleRefresh = (payload) => {
      if (!payload?.scope || payload?.scope === 'dean') loadDashboard()
    }
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link href="/dean/pending" className="text-sm font-medium text-emerald-600 hover:text-emerald-700 flex items-center gap-1">
            View All <ArrowRight className="h-3.5 w-3.5" />
          </Link>
        </div>

        {recentEvents.length === 0 ? (
          <p className="text-sm text-slate-400 py-8 text-center">No pending event requests.</p>
        ) : (
