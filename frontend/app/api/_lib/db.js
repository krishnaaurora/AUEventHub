...
// ─── Setup Guard ────────────────────────────────────────────────────────────
// Prevents redundant index/collection creation on every API request.
async function runSetupOnce(setupFn) {
  if (globalThis._mongoSetupDone) return
  if (setupInProgress) return setupInProgress

  const start = Date.now()
  setupInProgress = (async () => {
    try {
      console.log('--- [DB INFO] Starting MongoDB Setup Guard ---')
      await setupFn()
      globalThis._mongoSetupDone = true
      console.log(`--- [DB INFO] MongoDB Setup Completed in ${Date.now() - start}ms ---`)
    } finally {
...
export async function ensureStudentEventCollections() {
  return runSetupOnce(async () => {
    ...
    await events.createIndex({ status: 1, created_at: -1 })
