...
import { QrCode, Search, CheckCircle2, XCircle, Loader2, Clock, ShieldCheck, RefreshCw } from 'lucide-react'
...
export default function OrganizerAttendancePage() {
  ...
  return (
    <div className="space-y-6">
      <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="min-h-[200px]">
              {status === 'idle' && (
                <div className="rounded-3xl border border-slate-200 bg-slate-50/50 border-dashed p-10 text-center h-full flex flex-col items-center justify-center">
              ...
              {status === 'success' && (
                <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6 shadow-sm relative overflow-hidden transition-all duration-300">
                  ...
                  <button onClick={() => setStatus('idle')} className="mt-6 w-full py-2.5 rounded-xl bg-white text-emerald-700 text-sm font-bold hover:bg-emerald-100 transition-colors border border-emerald-200 shadow-sm">
                    Done
                  </button>
                </div>
              )}
              ...
              {status === 'error' && (
                <div className="rounded-3xl border border-rose-200 bg-rose-50 p-6 shadow-sm">
                  ...
                  <button onClick={() => setStatus('idle')} className="mt-6 w-full py-2.5 rounded-xl bg-white text-rose-700 text-sm font-bold hover:bg-rose-100 transition-colors border border-rose-200 shadow-sm">
                    Close
                  </button>
                </div>
              )}
              ...
              <div>
                ...
                <div className="flex justify-center gap-3">
                  {eventCode.split('').map((char, i) => (
                    <div 
                      key={i}
                      className="w-14 h-20 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-4xl font-black text-indigo-400 shadow-[0_0_20px_rgba(99,102,241,0.1)] transition-transform hover:scale-105"
                    >
                      {char}
                    </div>
                  ))}
                ...
                <div className="relative h-16 w-16">
                  <svg className="h-16 w-16 transform -rotate-90">
                    ...
                    <circle 
                      ...
                      strokeDashoffset={188.4 * (1 - timeLeft / 60)}
