...
let poolInstance
let memoryDb
let setupInProgress = null

function isMemoryDatabase() {
  return !process.env.DATABASE_URL
}

async function runSetupOnce(setupFn) {
  if (globalThis._pgSetupDone) return
  if (setupInProgress) return setupInProgress

  const start = Date.now()
  setupInProgress = (async () => {
    try {
      console.log('--- [PG INFO] Starting Postgres Setup Guard ---')
      await setupFn()
      globalThis._pgSetupDone = true
      console.log(`--- [PG INFO] Postgres Setup Completed in ${Date.now() - start}ms ---`)
    } finally {
...
export async function ensureStudentTransactionTables() {
  return runSetupOnce(async () => {
    ...
    // Cleanup duplicates before adding unique constraint (keep newest only)
    await pool.query(`
      DELETE FROM certificate_templates a 
      USING certificate_templates b 
      WHERE a.id < b.id AND a.event_id = b.event_id
    `).catch(() => {})
    
    // Force uniqueness on event_id for existing tables to support ON CONFLICT
    await pool.query('ALTER TABLE certificate_templates ADD UNIQUE (event_id)').catch(() => {})
    await pool.query('CREATE UNIQUE INDEX IF NOT EXISTS certificate_templates_event_id_unique_idx ON certificate_templates(event_id)')
    ...
    if (isMemoryDatabase()) {
      ...
      if (registrationCountResult.rows[0].count === 0) {
        for (const row of DEMO_SQL_SEED.registrations) {
          await pool.query(`INSERT INTO registrations (student_id, event_id, ticket_id, qr_code, status) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (student_id, event_id) DO NOTHING`, row)
        }
      }
      ...
      if (attendanceCountResult.rows[0].count === 0) {
        for (const row of DEMO_SQL_SEED.attendance) {
          await pool.query(`INSERT INTO attendance (student_id, event_id, status) VALUES ($1, $2, $3) ON CONFLICT (student_id, event_id) DO UPDATE SET status = EXCLUDED.status, scanned_at = CURRENT_TIMESTAMP`, row)
        }
      }
      ...
      if (certificateCountResult.rows[0].count === 0) {
        for (const row of DEMO_SQL_SEED.certificates) {
          await pool.query(`INSERT INTO certificates (student_id, event_id, certificate_url) VALUES ($1, $2, $3) ON CONFLICT (student_id, event_id) DO UPDATE SET certificate_url = EXCLUDED.certificate_url, issued_at = CURRENT_TIMESTAMP`, row)
        }
      }
      ...
      if (notificationCountResult.rows[0].count === 0) {
        for (const row of DEMO_SQL_SEED.notifications) {
          await pool.query(`INSERT INTO notifications (user_id, message, priority) VALUES ($1, $2, $3)`, row)
        }
      }
    }
  })
